...
const ShowList = (props) => {
  ...
  const [ lastPageNum, setLastPageNum ] = useState(232);
  // let nextPageNum = 0;
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        ...
        // const { data: nextPageData } = await axios.get(`https://api.tvmaze.com/shows?page=${props.match.params.pagenum + 1}`);
  ...
  if (loading) {
    ...
    return (
      <div>
        {props.match.params.pagenum === lastPageNum && (  // The last oge
        ...
        {!hasError && props.match.params.pagenum < lastPageNum && (
          <section className="App">
            <button onClick={() => { decrePageNum(); buttonChangeUrl();}}>   {/* https://upmostly.com/tutorials/multiple-onclick-events-in-react-with-examples#call-multiple-functions */}
              Previous Page
            </button> &nbsp;&nbsp;
