...
const ComicsList = (props) => {
  // 1. For valid url 
  // https://gateway.marvel.com:443/v1/public/comics?offset=20&apikey=129562f44ff4e796645e21a3e6fb4bb5
  ...
  const [ comicTerm, setComicTerm ] = useState('');
  ...
  useEffect(() => {  // search form keyword fire this
    ...
    async function fetchData() {
      try {
        console.log(`search: ${comicTerm}`);
        const { data } = await axios.get(baseUrl + '?titleStartsWith=' + comicTerm + '&ts=' + ts + '&apikey=' + publickey + '&hash=' + hash);
  ...
  const buttonChangeUrl = async () => {
    history.push(`/comics/page/${props.match.params.page}`);
  }
  ...
  function ErrorComponent() {
    return (
      <div>
        <h1>404 - Comic Not Found! You're out of comic List</h1>
      </div>
    )
  }
  ...
  if (comicTerm) {  // search keyword
    card = searchData && searchData.map((comic) => {
      return buildCard(comic);  // call buildCard() function to build card for each comic
    });
  
  } 
  else if (props.match.params.page && props.match.params.page >= 0) {
    card = pageData && pageData.map((comic) => {
      return buildCard(comic);
    });
  }
  else {
    card = initialData && initialData.map((comic) => {
      return buildCard(comic);
    });
  }
