  ...
  const [ removeEmployee ] = useMutation(queries.DELETE_EMPLOYEE, {
    update(cache, {data: { removeEmployee}} ) {
      ...
      cache.writeQuery({
        ...
        data: { employees: employees.filter((e) => e.id !== employee.id) }
      });
    }
  });
    ...
    <div>
      ...
      {/* Here we set up the mutation, since I want the data on the page to update
        ...
        <div>
          ...
          <form clasName="form" id="delete-employee" onSubmit={(e)=>{
            e.preventDefault()
            removeEmployee({
            ...
            <br />
            <br />
            <button className="button add-button" type="submit">Delete Employee</button>
          </form>
