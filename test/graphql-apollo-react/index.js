...
const lodash = require('lodash');  // for parsing data easier
...
// resolvers are function that perform your database stuff. For example, our data are coming from database instead of filtering the array. you will make a call to your database
const resolvers = {  
  Query: {
    employers: () => employers,
    employees: () => employees,
  ...
  // Once we have our query type definition, we need resolvers for our type
  Employer: {
  ...
  Mutation: {
    addEmployee: (_, args) => {
      const newEmployee = {id: uuid.v4(), firstName: args.firstName, lastName: args.lastName, employerId: args.employerId};
    ...
    editEmployee: (_, args) => {
      ...
      employees = employees.map((e) => {
        if (e.id === args.id) {  // find the employee who we want to edit
          if (args.firstName) e.firstName = args.firstName;
          if (args.lastName) e.lastName = args.lastName;
          if (args.employerId) e.employerId = args.employerId;
