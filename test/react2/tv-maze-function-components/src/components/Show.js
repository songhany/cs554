  ...
  useEffect(
    () => {
      async function fetchData() {
        try {
          const {data} = axios.get('http://api.tvmaze.com/shows/' + props.match.params.id)
          setShowData(data);
        } catch (e) {
          console.log(e);
        }
      ...
      fetchData();
    }, [props.match.params.id]  // when thie is a change to params.id in URL, then useEffect() function will fire
  );  
